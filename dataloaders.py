...
def get_dataloaders(dataset, data_path, batch_size, eval_batch_size, max_len, device):
    ...
    # (TODO): chech if train_iter is consumed? need?
    v_train = train_df.values
    train_iter = list(map(lambda x: (x.tolist()[1], x.tolist()[0]), v_train))
    ...
    #  if dataset == 'AGNEWS': # tutorial
        ...
        #  train_dataloader = DataLoader(train_dataset, batch_size=batch_size, shuffle=True, collate_fn=collate_batch)
        #  valid_dataloader = DataLoader(val_dataset, batch_size=eval_batch_size, shuffle=False, collate_fn=collate_batch)
        #  test_dataloader = DataLoader(test_dataset, batch_size=eval_batch_size, shuffle=False, collate_fn=collate_batch)
